...
Client-side routing is when the url changes but a request to the server isn't made because the route is handled internally but the JS that is already loaded on the page. The main benefits of this is that less data is processed and routing between pages is generally faster. Smooth transitions and animations between views are easier to implement.
